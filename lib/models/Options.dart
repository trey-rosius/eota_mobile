...
/** This is an auto generated class representing the Options type in your schema. */
class Options {
  final List<Option>? _items;
  ...
  const Options._internal({required items}): _items = items;
  
  factory Options({required List<Option> items}) {
    return Options._internal(
      items: items != null ? List<Option>.unmodifiable(items) : items);
  }
  ...
  Options copyWith({List<Option>? items}) {
    return Options._internal(
      items: items ?? this.items);
  }
  
  Options copyWithModelFieldValues({
    ModelFieldValue<List<Option>>? items
  }) {
    return Options._internal(
  ...
  Map<String, dynamic> toJson() => {
    'items': _items?.map((Option? e) => e?.toJson()).toList()
  };
  ...
  static var schema = amplify_core.Model.defineSchema(define: (amplify_core.ModelSchemaDefinition modelSchemaDefinition) {
    modelSchemaDefinition.name = "Options";
